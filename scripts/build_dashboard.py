...
def build_plan_why(row):
    ...
    if add_signal == "ADD SMALL":
        return f"Add small ${buy_usd:.2f}: {add_reason}. {timing}. {quality}. {reasons}"

    if action in ["BUY", "BUY SMALL"]:
        return f"{action} ${buy_usd:.2f}: {tier}. {timing}. {quality}. {reasons}"

    if action == "HOLD":
        if add_reason:
            return f"Hold. {add_reason}. {timing}. {quality}. {warnings or reasons}"
        return f"Hold. {timing}. {quality}. {warnings or reasons}"

    if base_action in ["BUY", "BUY SMALL"] and action == "WATCH":
        return f"Wait — signal was {base_action}, but not trade-ready. {timing}. {quality}. {warnings or reasons}"

    if review_note:
        return f"{review_note}. {timing}. {quality}. {warnings or reasons}"
